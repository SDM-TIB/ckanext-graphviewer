use crate::{AppState, constants, graph_processor};
use eframe::egui;
use log::debug;
...
pub fn fetch_keyword_information(
    ...
    // Construct the URL matching the FastAPI endpoint
    let url = format!(
        "{}/get_dataset_information_by_keyword?keyword={}",
        api_url,
    ...
    ehttp::fetch(request, move |response| {
        if let Ok(res) = response {
            if let Some(text) = res.text() {
                // The API returns {"keyword": ..., "results": ...} which parse_dynamic_api_json handles
                ...
                } = &mut *state_lock
                {
                    ...
                    // Merge new triples and rebuild UI graph
                    raw_triples.extend(new_triples);
                    let (mut new_nodes, mut new_edges) = graph_processor::build_ui_graph(raw_triples.clone());

                    // Normalize orientation if dataset points to author (preserves existing logic)
                    ...
                    // Identify newly connected neighbor nodes
                    ...
                    // Restore visibility, positions, and track new nodes to layout
                    ...
                    // Position newly expanded nodes radially around the clicked node
